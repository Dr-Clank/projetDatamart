...
SELECT BUSINESS_ID, AVG(ABS(TO_DATE(FIRST_DATE,'yyyy/mm/dd hh24:mi:ss')-TO_DATE(PREV_DATE,'yyyy/mm/dd hh24:mi:ss'))) as interval_moyen from (
  SELECT CH_B_ID as BUSINESS_ID, ch_date as FIRST_DATE,
    LAG(ch_date, 1, 0) OVER (PARTITION BY FB_B_ID ORDER BY FB_B_ID,ch_date) AS PREV_DATE
    FROM 
    (
        SELECT ch.BUSINESS_ID as CH_B_ID, fb.BUSINESS_ID as FB_B_ID, ch."DATE" as ch_date
        FROM
        CHECKIN ch INNER JOIN FOOD_BUSINESS fb on fb.BUSINESS_ID = ch.BUSINESS_ID
        group by ch.BUSINESS_ID,fb.BUSINESS_ID, ch."DATE"
    )
  )
  WHERE PREV_DATE <> '0'
